...
class ModelManager:
    ...
    def generate_response(
        ...
        try:
            logger.info(f"[模型推理] 开始生成回复...")
            logger.info(f"[模型推理] 提示词长度: {len(prompt)} 字符")
            logger.info(f"[模型推理] 历史对话轮数: {len(history) if history else 0}")
            
            # 构建完整的输入
            if "chatglm" in self.model_name.lower():
                # ChatGLM 系列
                logger.info("[模型推理] 使用 ChatGLM 推理模式")
                # 如果有系统提示词，需要特殊处理
                if system_prompt:
                    # 将系统提示词添加到第一轮对话
                    full_prompt = f"{system_prompt}\n\n用户：{prompt}\n助手："
                    logger.info("[模型推理] 已添加系统提示词")
                else:
                    full_prompt = prompt
                ...
                logger.info(f"[模型推理] 准备调用 model.chat()...")
                logger.info(f"[模型推理] 输入提示词预览: {full_prompt[:200]}..." if len(full_prompt) > 200 else f"[模型推理] 输入提示词: {full_prompt}")
                logger.info(f"[模型推理] 历史对话轮数: {len(history) if history else 0}")
                logger.info(f"[模型推理] 生成参数: {chat_config}")
                logger.info(f"[模型推理] 使用设备: {self.device}")
                
                # 检查模型状态
                if hasattr(self.model, 'training'):
                    logger.info(f"[模型推理] 模型训练模式: {self.model.training}")
                
                inference_start = time.time()
                max_new_tokens = chat_config.get("max_new_tokens", 1024)
                logger.info(f"[模型推理] 开始调用 model.chat()（最大生成 {max_new_tokens} tokens）...")
                ...
                try:
                    ...
                    if use_streaming:
                        logger.info("[模型推理] 使用流式生成模式（可显示实时进度）")
                        last_response = ""
                        ...
                        with warnings.catch_warnings():
                            ...
                            with torch.no_grad():
                                for response, _ in self.model.stream_chat(
                                    self.tokenizer,
                                    full_prompt if not history else prompt,
                                    history=history or [],
                                    **chat_config
                                ):
                        ...
                        # 回退到非流式生成
                        logger.info("[模型推理] 使用标准生成模式（无法显示实时进度）")
                        # 抑制 ChatGLM 内部的警告（其日志格式化有 bug）
                        import warnings
                        with warnings.catch_warnings():
                            warnings.filterwarnings("ignore", category=UserWarning)
                    ...
                    logger.info(f"[模型推理] ✅ ChatGLM 推理完成")
                    logger.info(f"[模型推理] 总耗时: {inference_time:.2f} 秒")
                    logger.info(f"[模型推理] 生成 tokens: {final_tokens}")
                    logger.info(f"[模型推理] 平均速度: {final_speed:.2f} tokens/秒")
                    logger.info(f"[模型推理] 生成回复长度: {len(response)} 字符")
                    logger.info(f"[模型推理] 回复预览: {response[:200]}..." if len(response) > 200 else f"[模型推理] 回复: {response}")
                    return response
                ...
                # 添加历史对话
                if history:
                    logger.info(f"[模型推理] 添加 {len(history)} 轮历史对话")
                ...
                # 应用对话模板
                logger.info("[模型推理] 应用对话模板...")
                text = self.tokenizer.apply_chat_template(
                    messages,
                    tokenize=False,
                    add_generation_prompt=True
                )
                logger.info(f"[模型推理] 模板文本长度: {len(text)} 字符")
                
                # 编码
                logger.info("[模型推理] 编码输入...")
                encode_start = time.time()
                inputs = self.tokenizer.encode(text, return_tensors="pt").to(self.device)
                encode_time = time.time() - encode_start
                logger.info(f"[模型推理] 编码完成，耗时: {encode_time:.2f} 秒，输入长度: {inputs.shape[1]} tokens")
                
                # 生成
                logger.info("[模型推理] 开始生成（这可能需要一些时间）...")
                logger.info(f"[模型推理] 生成参数: {GENERATION_CONFIG}")
                gen_start = time.time()
                with torch.no_grad():
                ...
                logger.info(f"[模型推理] 输出 tokens 数量: {outputs.shape[1] - inputs.shape[1]}")
                
                # 解码
                logger.info("[模型推理] 解码输出...")
                ...
                logger.info(f"[模型推理] 解码完成，耗时: {decode_time:.2f} 秒")
                logger.info(f"[模型推理] 最终回复长度: {len(response)} 字符")
